...
#include <stdbool.h>
#include "constants.h"
#include "uartFunctions.h"         /* For true/false definition                     */
...
#ifdef DEBUG
    sendDataBuffer("Generated hardware execption\n");

    if(_excep_code == EXCEP_AdES)
    {
        sendDataBuffer("Segmentation fault (store)\n");
    }
    if(_excep_code == EXCEP_AdEL)
    {
        sendDataBuffer("Segmentation fault (read)\n");
    }
    else if(_excep_code == EXCEP_IRQ)
    {
        sendDataBuffer("Interuption error\n");
    }
    else if(_excep_code == EXCEP_Trap)
    {
        sendDataBuffer("Trap Error\n");
    }
    else
    {
        sendDataBuffer("Unknow cause\n");
    }
